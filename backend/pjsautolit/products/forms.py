...
class DiscountForm(forms.ModelForm):
    start_date = forms.DateField(
        ...
        initial=timezone.now().date,  # returns only the date
    )
    ...
    def clean_product_tags(self):
        ...
        if apply_to == "SPECIFIC_PRODUCTS":
            if not product_tags:
                raise forms.ValidationError(
                    "Product tags are required when applying to specific products."
                )
        ...
        if product_tags:
            ...
            tags = [
                tag.strip().lower() for tag in product_tags.split(",") if tag.strip()
            ]
            ...
            for tag in tags:
                ...
                if len(tag) > 100:  # Optional: Add a max length for tags
                    raise forms.ValidationError(
                        f"Tag '{tag}' is too long. Maximum 100 characters."
                    )
    ...
    def clean_name(self):
        ...
        if discount_type == "COUPON":
            ...
            if (
                Discount.objects.filter(name=name)
